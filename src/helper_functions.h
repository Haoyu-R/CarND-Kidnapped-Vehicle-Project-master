...
inline double Gaussian2D(double obs_x, double obs_y, double mu_x, double mu_y, double sigma_x, double sigma_y) {
   
    double gaussian_norm = 1 / (2 * M_PI * sigma_x * sigma_y);
    double expontnent = -0.5 * (pow((obs_x - mu_x) / sigma_x, 2) + pow((obs_y - mu_y) / sigma_y, 2));
    return gaussian_norm * exp(expontnent);
}
